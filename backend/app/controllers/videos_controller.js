...
exports.getVideos = async (req, res) => {
    try {
        ...
        if (req.body.club === "" && req.body.option === "") {
            ...
            const videos = await Video.find({ $text: { $search: name + " " + req.body.option } }, { score: { $meta: "textScore" } }).sort({ score: { $meta: "textScore" } });
            return res.status(200).json({ status: 'ok', videos })
        } else if (req.body.club === "") {
            const videos = await Video.find({ $text: { $search: name + " " + req.body.club } }, { score: { $meta: "textScore" } }).sort({ score: { $meta: "textScore" } });
            return res.status(200).json({ status: 'ok', videos })
        } else {
            const videos = await Video.find({ $text: { $search: name + " " + req.body.option + " " + req.body.club } }, { score: { $meta: "textScore" } }).sort({ score: { $meta: "textScore" } });
            return res.status(200).json({ status: 'ok', videos })
        }
